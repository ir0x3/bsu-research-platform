import { searchResearch } from "./firebaseSearch";
...
export async function generateGeminiResponse(
  ...
  //  1. البحث في Firebase الأول
  try {
    const results = await searchResearch(prompt);

    if (results.length > 0) {
      console.log("Found results in Firebase:", results);
...
`
  ...
  if (!API_KEY) {
    ...
    return "⚠️ لم يتم إعداد مفتاح الذكاء الاصطناعي. يرجى مراجعة إعدادات المنصة.";
  }
    ...
    contents: [
      {
        ...
        parts: [
          {
            text: `أنت مساعد بحثي أكاديمي ذكي لمنصة "أبحاث قسم علوم المعلومات" (BSU Research Platform). 
...
تأكد من أن جميع إجاباتك تتبع قواعد اللغة العربية الصحيحة.`,
          },
        ],
      },
      {
        ...
        parts: [
          {
            text: "فهمت، أنا المساعد الأكاديمي الذكي لمنصة أبحاث قسم علوم المعلومات. كيف يمكنني مساعدتك اليوم؟",
          },
        ],
      },
      ...history,
    ...
    try {
      ...
      if (response.ok) {
        ...
        return (
          ...
          "عذراً، لم أتمكن من تكوين إجابة حالياً."
        );
      }

      const err = await response.json();
      console.error(
        `Gemini API Error (${version}/${model}, status ${response.status}):`,
  ...
  return "❌ لا توجد نتائج حالياً. حاول مرة أخرى لاحقاً.";
}